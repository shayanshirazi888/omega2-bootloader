            ...
            if (ms_count >= 50) 
            {
                ...
                if (sqfs_start > 0) {
                    ...
                    if (rootfs_data_offset > 0x100000 && rootfs_data_offset < flash_size) {
                        ...
                        #if defined(CFG_ENV_IS_IN_SPI)
                            raspi_erase(rootfs_data_offset, rootfs_data_len);
                        #else
                            flash_sect_erase(rootfs_data_start, CFG_FLASH_BASE + flash_size - 1);
                        #endif
                        
                        printf("[PGP] Factory Reset Complete!\n");
                    } else {
                        printf("[!] ERROR: Calculated offset invalid. Aborting wipe.\n");
                    }
                } else {
                    printf("[!] ERROR: SquashFS magic not found. Aborting to prevent damage.\n");
                }
